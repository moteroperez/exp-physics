...
def plot(X, Y, Z, points_x, points_y, points_z, var, cte_R, cte_C, cte_V):
    ...
    ax.set_title(f'Intensidad en función del tiempo y {varied_label}', fontsize=4)
    ...
    plt.tight_layout()
    plt.savefig(f"media/var-{var}.png", bbox_inches='tight', dpi=300)
    plt.show()
...
# Adjust time and y range based on empirical data
...
T, Y = np.meshgrid(t, y)
Z = f(T, cte_V, Y, cte_C)
plot(T, Y, Z, points_x, points_y, points_z, 'R', cte_R, cte_C, cte_V)
...
T, Y = np.meshgrid(t, y)
Z = f(T, cte_V, cte_R, Y)
plot(T, Y, Z, points_x, points_y, points_z, 'C', cte_R, cte_C, cte_V)
...
T, Y = np.meshgrid(t, y)
Z = f(T, Y, cte_R, cte_C)
plot(T, Y, Z, points_x, points_y, points_z, 'V', cte_R, cte_C, cte_V)
